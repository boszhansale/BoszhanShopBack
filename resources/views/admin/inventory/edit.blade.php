            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="store_id" class="form-control">
                        @foreach($inventory->salesrep->stores()->orderBy('name')->get() as $store)
                            <option
                                {{$store->id == $inventory->store_id ? 'selected':''}} value="{{$store->id}}">{{$store->name}}
                ...
                <div class="form-group">
                    ...
                    <select name="salesrep_id" class="form-control" required>
                        @foreach($salesreps as $salesrep)
                            <option
                                {{$inventory->salesrep_id == $salesrep->id ? 'selected':''}} value="{{$salesrep->id}}">{{$salesrep->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="driver_id" class="form-control" required>
                        @foreach($drivers as $driver)
                            <option
                                {{$inventory->driver_id == $driver->id ? 'selected':''}}  value="{{$driver->id}}">{{$driver->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status_id" class="form-control" required>
                        @foreach($statuses as $status)
                            <option
                                {{$inventory->status_id == $status->id ? 'selected':''}}  value="{{$status->id}}">{{$status->description}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="payment_type_id" class="form-control" required>
                        @foreach($paymentTypes as $paymentType)
                            <option
                                {{$inventory->payment_type_id == $paymentType->id ? 'selected':''}}  value="{{$paymentType->id}}">{{$paymentType->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="payment_status_id" class="form-control" required>
                        @foreach($paymentStatuses as $paymentStatus)
                            <option
                                {{$inventory->payment_status_id == $paymentStatus->id ? 'selected':''}}  value="{{$paymentStatus->id}}">{{$paymentStatus->name}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="delivery_date" class="form-control" required
                           value="{{\Carbon\Carbon::parse($inventory->delivery_date)->format('Y-m-d')}}">
                </div>
                <div class="form-group">
                    <label for="">Дата доставлено</label>
                    <input type="datetime-local" name="delivered_date" class="form-control" required
                           value="{{\Carbon\Carbon::parse($inventory->delivered_date)->format('Y-m-d H:i')}}">
                </div>
                <div class="form-group">
